...
compare_enrichment <- function(boolean_input_df, gene_coordinates_df, primary_input_entities, combo_length, min_indv_threshold, max_freq_threshold, input_format = 'Input_', output_format = 'Output_', pval_filter_threshold = 0.05, adj_pval_type = 'BH', min_power_threshold = 0.7, sample_names_ind = 'N', ld_block_size=0, quiet=T) {
    ...
    ###########################################################
    # remove combos without an item in primary_input_entities #
    ###########################################################


    rows_to_keep = case_freqitems_df[,'Item_1'] %in% primary_input_entities
    for (i in 1:combo_length) {
    rows_to_keep = (rows_to_keep) | (case_freqitems_df[,glue('Item_{i}')] %in% primary_input_entities)
    }
    ...
    if (!quiet) {
        print(paste0('Number of combinations identified for cases: ', dim(case_freqitems_df)[1]))
